...
\end{equation}
dove $\sim \in \{\leq, \geq, =\}$.
Rispetto alla \ref{eq:lpDef} viene introdotto in aggiunta il vincolo che alcune
delle variabili devono assumere valori interi.
...
\begin{enumerate}
    ...
    \item Si effettua \emph{branching} sul nodo radice.
        ...
        uno dei due seguenti vincoli aggiuntivi:
        \begin{equation}
...
ma non consente di determinare se la soluzione trovata è la migliore possibile
...
L'ACS ricava una soluzione ammissibile per MIP risolvendo molteplici
sub-MIP con il B\&C.
...
un algoritmo esatto per velocizzare il processo di risoluzione di un
generico problema MIP.
Infatti l'implementazione allo stato dell'arte di B\&C \cite{cplex22.1.1}
utilizza degli euristici per ottenere soluzioni utili per la dimostrazione 
di ottimalità.
L'utilizzo di questi metodi permette in molti casi di ottenere soluzioni
di alta qualità più velocemente rispetto a sfruttare solamente il branching.
